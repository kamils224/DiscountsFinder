...
def _build_testing_paths() -> List[Tuple[str, str]]:
    shop_dirs = [
        os.path.join(SAMPLES_DIR, shop_directory_name)
        for shop_directory_name in os.listdir(SAMPLES_DIR)
    ]
    results = []
    for shop_path in shop_dirs:
        for sample in os.listdir(shop_path):
            ...
            ground_truth_path = os.path.join(
                GROUND_TRUTH_DIR, os.path.basename(shop_path), f"{name}.csv"
            )
